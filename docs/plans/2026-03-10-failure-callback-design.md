...
- 出于安全与性能考虑，失败回调不传 payload，只传元数据（避免敏感数据泄露）。

## 应用层示例（可选）

在 `apps/minirpc/parent` 提供一个轻量的失败监控器示例（非框架强制依赖）：
- `FailureMonitor` 接收 `RpcFailure` 回调流。
- 只统计 `ExecTimeout`（或同时统计 `PeerDisconnected/ProtocolMismatch`）。
- 支持滑动窗口阈值（例如 1 分钟 3 次）。
- 达到阈值后触发用户提供的 `OnThreshold` 回调，由应用决定是否重启子进程。

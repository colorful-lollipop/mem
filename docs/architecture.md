...
`CurrentControl()` 会优先从 `bootstrapChannel_` 取当前 control；失败时再退回 `fallbackControl_`，最后才调用 loader 重新加载。

这反映的设计是：

- 控制面连接不是每次调用都强制重建
- 但在 engine death / control 失效后，下一次显式需要时可以按需刷新
...
- `BuildControlLoader()`
  负责把“如何拿到系统能力 remote 对象”封装成一个可重试的 loader。首次优先使用已有 remote；后续可通过 SA manager 重新加载。
- `CurrentControl()`
  负责在运行时获取当前可用的 control proxy，并在 bootstrap channel 失效时退回到缓存或重新加载。

把这三者连起来看，就能明白 `VesClient` 处理控制面的思路不是“每次临时现连”，而是：

- 初始化阶段建立一个尽量稳定的 control 获取机制。
- 运行阶段优先用 bootstrap channel 当前持有的 control。
- control 失效后允许平滑退回缓存，再在需要时显式重建。

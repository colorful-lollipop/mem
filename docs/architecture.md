...
`CurrentControl()` 会优先从 `bootstrapChannel_` 取当前 control；只有 channel 不存在时才调用 loader 重新加载。
...
- `CurrentControl()`
  负责在运行时获取当前可用的 control proxy；正常情况下只信任 `bootstrapChannel_`，channel 不存在时才重新加载。

把这三者连起来看，就能明白 `VesClient` 处理控制面的思路不是“每次临时现连”，而是：

- 初始化阶段建立一个尽量稳定的 control 获取机制。
- 运行阶段只通过 bootstrap channel 获取当前 control。
- channel 生命周期结束后，如仍显式需要 control，再通过 loader 重建。

...
const std::string REGISTRY_SOCKET = "/tmp/vpsdemo_stress_registry.sock";
const std::string SERVICE_SOCKET = "/tmp/vpsdemo_stress_service.sock";
...
void WorkerThread(const StressConfig& config, uint32_t threadId,
    ...
    for (int i = 0; i < config.iterations && !g_stop; i++) {
        ...
        const auto behavior = vpsdemo::EvaluateSamplePath(path);

#ifndef VPSDEMO_REGISTRY_SERVER_H_
#define VPSDEMO_REGISTRY_SERVER_H_
...
// Callback for LOAD requests when SA is not registered.
using LoadCallback = std::function<bool(int32_t sa_id)>;
// Callback for UNLOAD requests.
using UnloadCallback = std::function<void(int32_t sa_id)>;

// Runs the SA registry over a Unix domain socket.
class RegistryServer {
 public:
    ...
    // Called by supervisor after spawning engine to register its socket path.
...
#endif  // VPSDEMO_REGISTRY_SERVER_H_

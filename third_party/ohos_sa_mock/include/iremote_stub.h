...
public:
    ...
    sptr<IRemoteObject> AsObject() override
    {
        if (remote_ == nullptr) {
            ...
            auto broker = std::dynamic_pointer_cast<IRemoteBroker>(RefBase::shared_from_this());
            auto self = std::dynamic_pointer_cast<IRemoteStub<T>>(RefBase::shared_from_this());
            wptr<IRemoteStub<T>> weak_self = self;
            remote_->AttachBroker(broker);
            remote_->SetRequestHandler([weak_self](int cmd, const MockIpcRequest& request, MockIpcReply* reply) {

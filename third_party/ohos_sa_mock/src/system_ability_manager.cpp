...
public:
    ErrCode AddSystemAbility(int32_t systemAbilityId, const sptr<IRemoteObject>& object) override
    {
        if (object == nullptr) {
...
public:
    ...
    sptr<IRemoteObject> LoadSystemAbility(int32_t systemAbilityId, int32_t timeout) override
    {
        const auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(timeout > 0 ? timeout : 0);
        while (true) {
...
SystemAbility::SystemAbility(int32_t systemAbilityId, bool runOnCreate)
    : system_ability_id_(systemAbilityId),
      run_on_create_(runOnCreate)
{
}
...
bool SystemAbility::Publish(const sptr<IRemoteObject>& object)
{
    ...
    if (!path.empty()) {
        if (!transport_.Start(path, [object](int cmd, const MockIpcRequest& request, MockIpcReply* reply) {
                return object->HandleRequest(cmd, request, reply);
            })) {
            return false;
        }
    }

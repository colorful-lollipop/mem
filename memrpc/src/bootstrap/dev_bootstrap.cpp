...
struct DevBootstrapChannel::Impl {
    ...
    {
        const int shm_fd = shm_open(config.shmName.c_str(), O_CREAT | O_RDWR | O_TRUNC, 0600);
        if (shm_fd < 0) {
            HILOGE("shm_open failed, name=%{public}s errno=%{public}d", config.shmName.c_str(), errno);
        }
        return shm_fd;
    }

...
bool ByteReader::ReadBytes(void* data, uint32_t size) {
  ...
  if (bytes_ == nullptr) {
    HILOGE("ByteReader::ReadBytes failed: source is null offset=%{public}zu size=%{public}u",
           offset_, size);
    return false;
  }
...
bool ByteReader::ReadBytesView(uint32_t size, ByteView* value) {
  ...
  if (bytes_ == nullptr) {
    HILOGE("ByteReader::ReadBytesView failed: source is null offset=%{public}zu size=%{public}u",
           offset_, size);
    return false;
  }
  if (offset_ + size > size_) {

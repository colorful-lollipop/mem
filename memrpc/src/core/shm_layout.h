...
{
    return cursor.tail.load(std::memory_order_relaxed) - cursor.head.load(std::memory_order_relaxed);
}
...
{
    return max_request_bytes > 0 && max_response_bytes > 0 &&
...
inline Layout ComputeLayout(const LayoutConfig& config)
{
    ...
    layout.normalRingOffset =
        AlignOffset(layout.highRingOffset + sizeof(RequestRingEntry) * config.highRingSize, alignof(RequestRingEntry));

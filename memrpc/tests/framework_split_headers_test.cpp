...
TEST(FrameworkSplitHeadersTest, MainlineHeadersDoNotDependOnLegacyCompatPaths)
{
    ...
    std::string clientContent((std::istreambuf_iterator<char>(clientHeader)), std::istreambuf_iterator<char>());
    ...
    std::ifstream testkitHeader(MEMRPC_REPO_PATH("/virus_executor_service/include/testkit/testkit_client.h"));
    ASSERT_TRUE(testkitHeader.is_open());
    std::string testkitContent((std::istreambuf_iterator<char>(testkitHeader)), std::istreambuf_iterator<char>());

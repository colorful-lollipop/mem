...
{
    PerfStats stats;
    stats.ops_per_sec = duration_sec > 0.0 ? static_cast<double>(ops) / duration_sec : 0.0;
...
{
    return std::min<std::size_t>(MemRpc::DEFAULT_MAX_REQUEST_BYTES, MemRpc::DEFAULT_MAX_RESPONSE_BYTES);
}

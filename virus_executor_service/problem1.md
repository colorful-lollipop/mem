...
`virus_executor_service_dt_crash_recovery` 中：

- **第一次 crash → 恢复 → 正常调用：OK**（Step 2-4 全部通过）
- **第二次 crash → 恢复后调用：挂住**（Step 5 卡死）

压测 `virus_executor_service_stress_client --iterations 100`（含 5% crash）也会卡住，根因应该一致。

## 复现

```bash
cmake --build virus_executor_service/build
timeout 20 ./build/virus_executor_service/virus_executor_service_dt_crash_recovery
```

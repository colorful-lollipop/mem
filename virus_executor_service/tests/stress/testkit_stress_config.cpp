...
std::vector<std::size_t> DefaultPayloadSizes(uint32_t maxRequestBytes) {
    ...
    sizes.erase(
        std::remove_if(
            sizes.begin(), sizes.end(), [safeMax](std::size_t value) { return value > safeMax; }),
        sizes.end());
    if (sizes.empty()) {
        sizes.push_back(0);
    }
...
StressConfig ParseStressConfigFromEnv() {
    ...
    config.highPriorityPct =
        GetEnvInt("MEMRPC_STRESS_HIGH_PRIORITY_PCT", config.highPriorityPct);
    config.burstIntervalMs =
        GetEnvInt("MEMRPC_STRESS_BURST_INTERVAL_MS", config.burstIntervalMs);
    config.burstDurationMs =
        GetEnvInt("MEMRPC_STRESS_BURST_durationMs", config.burstDurationMs);
    config.burstMultiplier =
        GetEnvInt("MEMRPC_STRESS_BURST_MULTIPLIER", config.burstMultiplier);
    config.noProgressTimeoutSec =
        GetEnvInt("MEMRPC_STRESS_NO_PROGRESS_SEC", config.noProgressTimeoutSec);
    config.maxRequestBytes = static_cast<uint32_t>(GetEnvInt(
        "MEMRPC_STRESS_MAX_REQUEST_BYTES", static_cast<int>(config.maxRequestBytes)));
    config.maxResponseBytes = static_cast<uint32_t>(GetEnvInt(
        "MEMRPC_STRESS_MAX_RESPONSE_BYTES", static_cast<int>(config.maxResponseBytes)));

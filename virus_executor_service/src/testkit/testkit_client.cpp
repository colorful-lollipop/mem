...
MemRpc::StatusCode TestkitClient::Sleep(uint32_t delayMs,
                                        SleepReply* reply,
                                        ...
                                        uint32_t execTimeoutMs)
{
    ...
    const MemRpc::StatusCode status = asyncClient_.SleepAsync(request, priority, execTimeoutMs).Wait(reply);
    if (status != MemRpc::StatusCode::Ok) {
        HILOGE("TestkitClient::Sleep failed status=%{public}d delay_ms=%{public}u", static_cast<int>(status), delayMs);
    }
    return status;
}

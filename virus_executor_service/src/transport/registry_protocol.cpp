...
bool EncodeRegistryRequest(const RegistryRequest& req, std::string* out) {
    if (out == nullptr || req.payload.size() > 0xFFFF) {
        HILOGE("EncodeRegistryRequest failed: out=%{public}p payload_size=%{public}zu",
            out, req.payload.size());
        return false;
    }
...
bool DecodeRegistryRequest(const uint8_t* data, size_t len, RegistryRequest* req) {
    if (data == nullptr || req == nullptr || len < 7) {
        HILOGE("DecodeRegistryRequest failed: data=%{public}p req=%{public}p len=%{public}zu",
            data, req, len);
        return false;
    }
    ...
    if (len < 7u + plen) {
        HILOGE("DecodeRegistryRequest failed: len=%{public}zu payload_len=%{public}u",
            len, plen);
        return false;
    }
...
bool EncodeRegistryResponse(const RegistryResponse& resp, std::string* out) {
    if (out == nullptr || resp.payload.size() > 0xFFFF) {
        HILOGE("EncodeRegistryResponse failed: out=%{public}p payload_size=%{public}zu",
            out, resp.payload.size());
        return false;
    }
...
bool DecodeRegistryResponse(const uint8_t* data, size_t len, RegistryResponse* resp) {
    if (data == nullptr || resp == nullptr || len < 6) {
        HILOGE("DecodeRegistryResponse failed: data=%{public}p resp=%{public}p len=%{public}zu",
            data, resp, len);
        return false;
    }
    ...
    if (len < 6u + plen) {
        HILOGE("DecodeRegistryResponse failed: len=%{public}zu payload_len=%{public}u",
            len, plen);
        return false;
    }
...
bool SendMessage(int fd, const std::string& msg) {
    ...
    if (send(fd, &len, 4, MSG_NOSIGNAL) != 4) {
        HILOGE("SendMessage failed sending length: fd=%{public}d len=%{public}u errno=%{public}d",
            fd, len, errno);
        return false;
    }
    size_t sent = 0;
    while (sent < msg.size()) {
        ...
        if (n <= 0) {
            HILOGE("SendMessage failed sending payload: fd=%{public}d sent=%{public}zu total=%{public}zu errno=%{public}d",
                fd, sent, msg.size(), errno);
            return false;
        }
...
bool RecvMessage(int fd, std::string* msg) {
    ...
    while (received < len) {
        ...
        if (n <= 0) {
            HILOGE("RecvMessage failed reading payload: fd=%{public}d received=%{public}zu len=%{public}u errno=%{public}d",
                fd, received, len, errno);
            return false;
        }
